...
class RtdetrReIDInterface:
    '''
    用于替换fastreid的ReID模型，使用RTDETR模型
    '''

    def __init__(self, config_file, weights_path, device, batch_size=16):
        super(RtdetrReIDInterface, self).__init__()
        if device != 'cpu':
            self.device = 'cuda'
        else:
            self.device = 'cpu'
        ...
        self.model = build_model(self.cfg)  # 默认为baseline
        self.model.eval()
    ...
    def inference(self, image, detections):
        ...
        for d in range(np.size(detections, 0)):  # iteration over detections
            ...
            patch = cv2.resize(patch, tuple(self.cfg.INPUT.SIZE_TEST[::-1]),
                               interpolation=cv2.INTER_LINEAR)  # [384, 128, 3]
    ...
    def inference_rtdetr(self, image, detections):
