...
cpdef eval_cuhk03_cy(float[:,:] distmat, long long[:] q_pids, long long[:]g_pids,
                     long long[:]q_camids, long long[:]g_camids, long long max_rank):
...
cdef void function_cumsum(cython.numeric[:] src, cython.numeric[:] dst, long long n):
    cdef long long i
    dst[0] = src[0]
    for i in range(1, n):
        dst[i] = src[i] + dst[i - 1]