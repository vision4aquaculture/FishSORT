...
from fast_reid.rtdetr_reid_interfece import RtdetrReIDInterface  # 从新建的rtdetr_reid_interfece.py中导入FastReIDInterface类
import copy
...
class Predictor(object):
    def __init__(
        ...
        if self.with_reid:
            ...
            self.rtdetr_reid_config = fast_reid_config  # 将原来的fast_reid_config替换为rtdetr_reid_config
            self.rtdetr_reid_weights = fast_reid_weights    # 将原来的fast_reid_weights替换为rtdetr_reid_weights
            # 将原来的FastReIDInterface类替换为新建的RtdetrReIDInterface类
            # self.encoder = FastReIDInterface(self.fast_reid_config, self.fast_reid_weights, 'cuda')
            self.encoder = RtdetrReIDInterface(self.rtdetr_reid_config, self.fast_reid_weights,'cuda')
    ...
    for frame_id, img_path in enumerate(files, 1):
        ...
        if outputs[0] is not None:
            ...
            online_im_detection = plot_tracking_detection(
                img_info['raw_img'], outputs[0][:, :4] / scale, (outputs[0][:, 4] * outputs[0][:, 5]),
...
def main(exp, args):
    ...
    #替换时保证predictor的输入与Predictor类的内部参数同时修改
    # predictor = Predictor(model, exp, trt_file, decoder, args.device, args.fp16,
    #                       with_reid=args.with_fastreid, fast_reid_config=args.fast_reid_config,
    #                       fast_reid_weights=args.fast_reid_weights)
    predictor = Predictor(model, exp, trt_file, decoder, args.device, args.fp16,
                          with_reid=args.with_fastreid, fast_reid_config=args.rtdetr_reid_config,
                          fast_reid_weights=args.fast_reid_weights)
