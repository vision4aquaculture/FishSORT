...
from .metaformer import MetaFormerBlock
...
__all__ = ['Ghost_HGBlock', 'Rep_HGBlock', 'DWRC3', 'C3_DWR', 'C2f_DWR', 'BasicBlock_Ortho', 'BottleNeck_Ortho',
           ...
           'C2f_ContextGuided',
           'ParallelAtrousConv', 'AttentionUpsample', 'AttentionDownsample', 'CSP_PAC', 'DGCST', 'DGCST2', 'RetBlockC3',
           'C3_RetBlock', 'C2f_RetBlock',
           'CAA_HSFPN', 'RepNCSPELAN4_CAA', 'C3_PKIModule', 'C2f_PKIModule', 'BasicBlock_FADC', 'BottleNeck_FADC',
           'C3_FADC', 'C2f_FADC', 'C3_PPA', 'C2f_PPA', 'FocusFeature',
           'SRFD', 'DRFD', 'CFC_CRB', 'SFC_G2', 'CAFM', 'CGAFusion', 'CAFMFusion', 'RGCSPELAN', 'C3_Faster_CGLU',
           'C2f_Faster_CGLU', 'BasicBlock_Faster_Block_CGLU', 'BottleNeck_Faster_Block_CGLU',
           'SDFM', 'PSFM', 'BasicBlock_Star', 'BottleNeck_Star', 'C3_Star', 'C2f_Star', 'C3_Star_CAA', 'C2f_Star_CAA',
           'C3_KAN', 'C2f_KAN', 'BasicBlock_KAN', 'BottleNeck_KAN', 'KANC3',
           ...
           'WTConv2d', 'RCM', 'PyramidContextExtraction',
           'DynamicInterpolationFusion', 'FuseBlockMulti', 'C2f_FMB', 'gConvC3', 'C2f_gConv', 'LDConv',
           'BasicBlock_WDBB', 'BottleNeck_WDBB', 'BasicBlock_DeepDBB', 'BottleNeck_DeepDBB',
           'C2f_AdditiveBlock', 'C2f_AdditiveBlock_CGLU', 'CSP_MSCB', 'EUCB', 'C2f_MSMHSA_CGLU', 'CSP_PMSFA',
           'C2f_MogaBlock', 'C2f_SHSA', 'C2f_SHSA_CGLU', 'C2f_SMAFB', 'C2f_SMAFB_CGLU',
           'DynamicAlignFusion', 'CSP_MutilScaleEdgeInformationEnhance', 'C2f_FFCM', 'C2f_SFHF', 'CSP_FreqSpatial',
           'C2f_MSM', 'CSP_MutilScaleEdgeInformationSelect', 'C2f_HDRAB', 'C2f_RAB',
           'LFEC3', 'MutilScaleEdgeInfoGenetator', 'ConvEdgeFusion', 'C2f_FCA', 'C2f_CAMixer', 'HyperComputeModule',
           'MANet', 'MANet_FasterBlock', 'MANet_FasterCGLU', 'MANet_Star', 'MultiScaleGatedAttn',
           'C2f_HFERB', 'C2f_DTAB', 'DTAB', 'C2f_JDPM', 'C2f_ETB', 'ETB', 'C2f_FDT', 'FDT', 'WFU', 'PSConv', 'C2f_AP',
           'ContrastDrivenFeatureAggregation', 'C2f_ELGCA', 'C2f_ELGCA_CGLU',
           'C2f_Strip', 'C2f_StripCGLU', 'MultiScalePCA', 'FSA', 'MultiScalePCA_Down', 'C2f_KAT', 'C2f_Faster_KAN',
...
class BottleNeck_FADC(BottleNeck):
    def __init__(self, ch_in, ch_out, stride, shortcut, act='relu', variant='d'):
        ...
        self.branch2b = AdaptiveDilatedConv(in_channels=ch_out, out_channels=ch_out, kernel_size=3, stride=stride,
                                            padding=1)
...
class Modulator(nn.Module):
    ...
    #
    #     return self.norm1(self.output_conv(combined_out))
...
class FreqSpatialFilter(FreqSpatial):
    def __init__(self, in_channels, cutoff=30, fw=0.5):
        super(FreqSpatialFilter, self).__init__(in_channels)
        self.cutoff = cutoff
        self.filter_weight = fw
        self.fft_conv = Conv(in_channels * 8, in_channels * 8, 3)
        self.fft_conv2 = Conv(in_channels * 8, in_channels * 8, 3)
...
class FCA(nn.Module):
    def __init__(self, dim, num_heads=4, reso=64, split_size=[2, 4], shift_size=[1, 2], expansion_factor=4.,
...
class MultiScalePCA_Down(nn.Module):
    def __init__(self, input_channel, gamma=2, bias=1):
        ...
        self.sigmoid = nn.Sigmoid()
        self.down = nn.Conv2d(in_channels=input_channel2, out_channels=input_channel1, kernel_size=3, stride=2,
                              padding=1)
...
class DynamicCIncMixerBlock_KAN(nn.Module):
    ...
    def forward(self, x):
        N, C, H, W = x.size()
        x = x + self.drop_path(self.layer_scale_1.unsqueeze(-1).unsqueeze(-1) * self.mixer(self.norm1(x)))
        x = x + self.drop_path(self.layer_scale_2.unsqueeze(-1).unsqueeze(-1) * self.mlp(
            self.norm2(x).flatten(2).permute(0, 2, 1)).permute(0, 2, 1).view([-1, C, H, W]).contiguous())
        return x
