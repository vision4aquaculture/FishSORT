...
from .attention import DAttention, HiLo, EfficientAdditiveAttnetion, DualDomainSelectionMechanism_IROS
...
__all__ = ['TransformerEncoderLayer_LocalWindowAttention', 'AIFI_LPE', 'TransformerEncoderLayer_DAttention',
           ...
           'TransformerEncoderLayer_Pola', 'DSMAttention']
...
class DSMAttention(nn.Module):
    ...
    def __init__(self, c1, num_heads=8, fc=256, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        # )
        #新增的模块
        from ultralytics.nn.extra_modules.block import FreqSpatialFilter
        self.FreqSpatialFilter = FreqSpatialFilter(fc // num_heads)
        self.dsm = DualDomainSelectionMechanism_IROS(fc//num_heads)
    def forward(self, x):
        # 频域变换
        f = self.FreqSpatialFilter(x)
        # embedding
        _, _, H, W = x.shape
        ...
        # Generate Q, K, V.
        qkvt = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1,
                                                                                         4)  # Shape: [3, B, h, N, Ch].
        qkvf = self.qkv(f).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1,
                                                                                         4)  # Shape: [3, B, h, N, Ch].
        ...
        x = self.dsm(factor_att, k_softmax_T_dot_v)
        # x = k_softmax_T_dot_v
        # x = torch.abs(x)
        # x_abs = x
        # x = self.gap(x)
